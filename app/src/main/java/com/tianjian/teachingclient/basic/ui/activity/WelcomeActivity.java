...
import com.tianjian.teachingclient.basic.service.DownloadFileService;
import com.tianjian.teachingclient.basic.ui.ThreadExecutor;
...
import com.tianjian.teachingclient.util.Utils;
	...
	* @author cheng
	...
	private void chechVersion(){
		...
		PackageInfo packInfo = new PackageInfo();
			try {
				packInfo = packageManager.getPackageInfo( "com.tianjian.dwms",0);
			} catch (Exception e) {
				e.printStackTrace();
		}
		...
		NetWorkHepler.postWsData("appUpdateWs", "process", request, new INetWorkCallBack() {
			...
			public void onResult(Object result) {
				...
				if(listdata !=null && listdata.get(0)!=null){
					 ...
					 float verson = Float.valueOf(StringUtil.isBlank(au.getVERSION())?"0":au.getVERSION());
					 float currentVerson = (float) 1.0;
					 try {
						 currentVerson = Float.valueOf(getPackageManager().getPackageInfo( "com.tianjian",0).versionName);
