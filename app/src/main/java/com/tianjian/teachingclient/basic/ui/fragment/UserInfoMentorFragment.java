...
import com.tianjian.teachingclient.basic.bean.InQueryAppUpdateSrv.DOCINVHISInQueryAppUpdateSrvOutputItem;
import com.tianjian.teachingclient.basic.bean.InQueryAppUpdateSrv.DOCINVHISInQueryAppUpdateSrvResponse;
import com.tianjian.teachingclient.basic.bean.InQueryCountSrv.InQueryCountSrvResponse;
import com.tianjian.teachingclient.basic.ui.activity.ChangePwdActivity;
import com.tianjian.teachingclient.basic.ui.activity.ContactAdminActivity;
import com.tianjian.teachingclient.basic.ui.activity.MyInfoActivity;
import com.tianjian.teachingclient.basic.ui.activity.MyStuOrMentorActivity;
...
import com.tianjian.teachingclient.util.ImageUtil;
...
public class UserInfoMentorFragment extends BaseFragment{
	...
	private void initView() {
		...
		questions_num = (TextView) rootView.findViewById(R.id.answer_num);
		consultation_num = (TextView) rootView.findViewById(R.id.answer_consultation_num);
		...
		refresh_layout = (SwipeRefreshLayout) rootView.findViewById(R.id.refresh_men_info_layout);
		...
		NetWorkHepler.postWsData("countWs", "process", hashMap, new INetWorkCallBack() {
			...
			public void onResult(Object result) {
				...
				if("Y".equals(response.getErrorFlag())){
					...
					tasks_num.setText(response.getInQueryCountSrvOutputCollection().getInQueryCountSrvOutputItem().get(0).getSENDCOUT());
					askme_num.setText(response.getInQueryCountSrvOutputCollection().getInQueryCountSrvOutputItem().get(0).getGETCOUT());
					questions_num.setText(response.getInQueryCountSrvOutputCollection().getInQueryCountSrvOutputItem().get(0).getREPLYCOUT());
					consultation_num.setText(response.getInQueryCountSrvOutputCollection().getInQueryCountSrvOutputItem().get(0).getREPLYCONSULTATIONCOUT());
				}else{
					ToastUtil.showToast(getActivity(), "查询数据失败！");
				}
